...
class TkCanvasRenderer:
    """Renders Scene primitives onto a Tkinter Canvas.

    This is the bridge between the toolkit-independent Scene and
    the Tkinter Canvas API. A future QtRenderer would implement
    the same interface using QGraphicsScene.
    """
    ...
    def render_primitive(self, primitive):
        """Render a single primitive onto the canvas.

        Args:
            primitive: A *Primitive instance.

        Returns:
            The canvas item ID, or None.
        """
        if isinstance(primitive, LinePrimitive):
            return self._render_line(primitive)
        elif isinstance(primitive, OvalPrimitive):
            return self._render_oval(primitive)
        elif isinstance(primitive, PolygonPrimitive):
            return self._render_polygon(primitive)
        elif isinstance(primitive, TextPrimitive):
            return self._render_text(primitive)
        elif isinstance(primitive, ImagePrimitive):
            return self._render_image(primitive)
        return None

    def render_scene(self, scene):
        """Render all primitives in a scene.

        Args:
            scene: A Scene instance.
        """
        for primitive in scene.all_primitives():
            item_id = self.render_primitive(primitive)
            primitive.item_id = item_id

    def _render_line(self, p):
        kwargs = {}
        if p.fill:
            kwargs["fill"] = p.fill
        if p.width is not None:
            kwargs["width"] = p.width
        if p.dash:
            kwargs["dash"] = p.dash
        if p.arrow:
            kwargs["arrow"] = p.arrow
        if p.cap:
            kwargs["cap"] = p.cap
        if p.tag:
            kwargs["tag"] = p.tag
        return self.canvas.create_line(p.coords, **kwargs)

    def _render_oval(self, p):
        kwargs = {}
        if p.outline:
            kwargs["outline"] = p.outline
        if p.fill:
            kwargs["fill"] = p.fill
        if p.width is not None:
            kwargs["width"] = p.width
        ...
        return self.canvas.create_oval(
            p.x1, p.y1, p.x2, p.y2, **kwargs)

    def _render_polygon(self, p):
        kwargs = {}
        if p.fill:
            kwargs["fill"] = p.fill
        if p.outline:
        ...
        if p.tag:
            kwargs["tag"] = p.tag
        return self.canvas.create_polygon(p.coords, **kwargs)

    def _render_text(self, p):
        kwargs = {}
        if p.fill:
            kwargs["fill"] = p.fill
        if p.anchor:
            kwargs["anchor"] = p.anchor
        if p.font:
            kwargs["font"] = p.font
        if p.tag:
            kwargs["tag"] = p.tag
        return self.canvas.create_text(p.x, p.y, text=p.text, **kwargs)

    def _render_image(self, p):
        kwargs = {}
        if p.anchor:
            kwargs["anchor"] = p.anchor
        if p.tag:
        ...
        return self.canvas.create_image(
            p.x, p.y, image=p.image_data, **kwargs)
